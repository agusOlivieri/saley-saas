...
import { createPromo } from '@/app/actions/promos';
...
export default function NuevaPromoPage() {
  ...
  const onSubmit = async (data: PromoFormValues) => {
    ...
    try {
      ...
      alert('Error inesperado al publicar la promoción.');
    } finally {
  ...
  return (
    <div className="pt-2 pb-10 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="text-sm font-semibold text-blue-900 hover:text-blue-750" id="btn-save-draft">
          Guardar borrador
        </button>
      </div>

      <div>
        <h2 className="text-2xl font-bold text-blue-950">Nueva promoción</h2>
        <p className="text-xs text-gray-500 mt-1">Creá una promo atractiva para tus clientes cercanos.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" id="form-nueva-promo">
        ...
        <div className="flex items-center justify-between bg-white border border-gray-100 p-4 rounded-xl shadow-sm">
          <div className="space-y-1">
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-bold text-blue-950">Activar promoción en tiempo real</span>
              <Info size={14} className="text-gray-400 cursor-pointer" />
            </div>
            <p className="text-[10px] text-gray-500">La promoción se mostrará a clientes cercanos ahora.</p>
          </div>
        ...
        <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex gap-4">
          <div className="w-16 h-16 bg-blue-100 rounded-xl flex items-center justify-center shrink-0 relative overflow-hidden">
            <div className="absolute w-10 h-10 rounded-full bg-orange-200 opacity-50 animate-ping"></div>
            <div className="z-10 w-8 h-8 bg-blue-950 rounded-full flex items-center justify-center border border-white">
              <MapPin size={16} className="text-white" />
            </div>
          </div>
          <div className="space-y-1">
            <h4 className="text-xs font-bold text-blue-950 flex items-center gap-1">
              Vista previa para clientes cercanos <Eye size={12} className="text-blue-900" />
            </h4>
            <p className="text-[10px] text-gray-500 leading-tight">Tu promoción se mostrará en la app Saley a usuarios que se encuentren cerca de tu local.</p>
            <p className="text-[10px] font-bold text-blue-900 flex items-center gap-1 mt-1">
              <MapPin size={10} /> Alcance estimado: ~1.2 km
            </p>
          </div>
        </div>
        ...
        >
          {isSubmitting ? (
            ...
            <><span className="text-lg leading-none">+</span> Publicar promoción</>
          )}
        </button>
